...
namespace Valve.VR.InteractionSystem
{
    ...
    public class Hand : MonoBehaviour
    {
		...
		[Header ("Raycaster")]
		public bool useRaycaster = false;
		public UnityEngine.EventSystems.MotionControlRaycaster raycaster;
		public CustomEvents.UnityEventSingleFloat RaycastEvent;

		[Header ("SteamVR Fallback Camera")]
        ...
        protected virtual void UpdateHovering()
        {
			...
			float closestUIDistance = float.MaxValue;
			Interactable closestUIInteractable = null;
			...
			if (useRaycaster && raycaster != null)
			{
				HandleRaycast (ref closestUIDistance, ref closestUIInteractable);
			}

			if (closestUIInteractable != null)
			{
				RaycastEvent.Invoke (closestUIDistance);
			}

            // Hover on this one
			hoveringInteractable = (closestUIInteractable!=null) ? closestUIInteractable : closestInteractable;
        }

		protected List<RaycastResult> m_RaycastResultCache = new List<RaycastResult>();
		protected PointerEventData pointerData;
		protected void HandleRaycast (ref float closestDistance, ref Interactable closestInteractable)
		{			
			...
			foreach (RaycastResult rayResult in m_RaycastResultCache)
			{
				if (rayResult.gameObject == null)
					continue;
				//we only want to count the ray results that came from our raycast
				bool isMyResult = rayResult.module.gameObject == this.raycaster.gameObject;
				if (!isMyResult)
					continue;

				Interactable interactable = rayResult.gameObject.GetComponentInParent<Interactable> ();
				bool didHitValidComponent = (interactable != null);

				if (didHitValidComponent)
				{
					float distance = rayResult.distance;	
        ...
        public GrabTypes GetGrabEnding(GrabTypes explicitType = GrabTypes.None)
        {
            if (explicitType != GrabTypes.None)
            {
				if (explicitType == GrabTypes.Pinch && grabPinchAction!=null && grabPinchAction.GetStateUp(handType))
                    return GrabTypes.Pinch;
            ...
            {
				if (grabPinchAction!=null && grabPinchAction.GetStateUp(handType))
                    return GrabTypes.Pinch;
